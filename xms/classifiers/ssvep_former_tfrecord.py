...
class Regression_Model:
    ...
    def __init__(self, output_directory, input_shape, output_shape, model_params, verbose=False, build=True, batch_size=64,
        ...
        if not "model_type" in model_params or not model_params["model_type"].lower() == "ssvep_former":
    ...
    def build_model(self, input_shape, output_shape, pre_model=None): 
        ...
        input_fft = tf.signal.fft(tf.cast(input_layer, dtype=tf.complex64))
        ...
        x = self.mlp_head(x, n_outputs=output_shape, output_mult=self.output_mult)
