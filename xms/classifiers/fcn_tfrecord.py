...
class Regression_Model:
    ...
    def build_model(self, input_shape, output_shape, pre_model=None): 
        ...
        for filt_idx, (n_filt, filt_sz) in enumerate(zip(self.nb_filters, self.filter_szs)):
            
            if self.dropout:
                x = tf.keras.layers.Dropout(0.2)(x)
            if filt_idx == 0:
                x = tf.keras.layers.Conv1D(n_filt, filt_sz, 1, padding='same')(x)
            else:
                x = tf.keras.layers.Conv1D(n_filt, filt_sz, 1, padding='same')(x)
            ...
            if filt_idx == 0:
                x = tf.keras.layers.Reshape((1, input_shape[0], n_filt))(x)
        
        full = tf.keras.layers.GlobalAveragePooling2D()(x)

        # Dense layer to calculate regressors
        output_layer = tf.keras.layers.Dense(output_shape, activation='softplus')(full)
